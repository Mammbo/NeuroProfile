...
Qdrant, unattended. One bad clip is logged and skipped; it never kills the run.
    ...
    download/normalize -> chunk_video -> [get_events_dataframe + predict]*chunks
      -> stitch -> reduce -> save .npz -> build_payload -> upsert_video

Model config: VIDEO on GPU (fp16), text + audio on CPU (see build_cfg). Matches the
proven test_encode.py config. Downloads cap at 480p.

Usage:
    python batch_encoding/batch_encode.py --corpus scripts/corpus.txt
    python batch_encoding/batch_encode.py https://... https://...
    python batch_encoding/batch_encode.py --corpus scripts/corpus.txt --limit 2
    python batch_encoding/batch_encode.py --corpus scripts/corpus.txt --overwrite
"""
import argparse
import hashlib
import json
...
# Cookies FILE (browser-cookie extraction fails on this box — keyring not unlocked).
...
def _vram_note():
    ...
    return f"{(total-free)/2**30:.2f}/{total/2**30:.2f}GiB"
...
# model config — VIDEO on GPU (fp16), text + audio on CPU
def build_cfg():
...
def download_url(url: str, work: Path, max_bytes: int):
    """ Returns (path, title, ytid). 480p cap + merge to mp4.
    Note: max_filesize only blocks formats that report their size ahead of time."""
    import yt_dlp
...
def resolve_source(source: str, work: Path, max_bytes: int):
    """Normalize one corpus line into (video_id, title, media_kind, media_path_or_text)."""
...
# encode
def assemble_transcript(events_and_starts):
    rows = {}
    for events, chunk_start in events_and_starts:
        if events is None or len(events) == 0:
            continue
        cols = set(getattr(events, "columns", []))
        text_col = "text" if "text" in cols else ("sentence" if "sentence" in cols else None)
        if text_col is None:
            continue
        for _, r in events.iterrows():
            t = int(round(chunk_start + float(r.get("start", 0.0))))
            txt = str(r.get(text_col, "")).strip()
            if txt:
                rows[t] = txt
    return " ".join(rows[t] for t in sorted(rows))
...
def encode_chunks(model, items, diag, empty_cache=True):
    chunk_predictions, events_and_starts = [], []
    for idx, (path, chunk_start) in enumerate(items):
        ...
        if idx == 1 and not diag["printed"] and segments:
            ...
            print(f"  >>> segment.start check: chunk1 seg[0].start={s0} "
                  f"chunk_start={chunk_start} => {verdict}")
            diag["printed"] = True

        chunk_predictions.append((preds, segments, chunk_start))
        events_and_starts.append((events, chunk_start))
        if empty_cache and torch.cuda.is_available():
            torch.cuda.empty_cache()
    return chunk_predictions, assemble_transcript(events_and_starts)


def encode_text(model, text, work):
    tf = work / "input.txt"
    tf.write_text(text)
    events = model.get_events_dataframe(text_path=str(tf))
    preds, segments = model.predict(events)
    return [(preds, segments, 0)], text


def encode_one(source, model, db, cfg, diag):
    ...
    if route == "__TEXT__":
        chunk_predictions, transcript = encode_text(model, payload_src, per)
    else:
        if route == "__URL__":
        ...
        chunk_predictions, transcript = encode_chunks(model, items, diag)
    ...
    dbpayload = storage.build_payload(
        ...
        moments=moments, timeline_path=timeline_name, transcript=transcript,
    )
    ...
    return {"status": "ok", "video_id": vid, "title": title,
            ...
            "n_moments": len(moments), "vram": _vram_note()}
...
def main():
    ...
    print(f"Corpus: {len(sources)} source(s). Log -> {logf}")
    ...
    for i, src in enumerate(sources, 1):
        print(f"\n=== [{i}/{len(sources)}] {src[:80]}  (vram {_vram_note()})")
        try:
            rec = encode_one(src, model, db, cfg, diag)
        except torch.cuda.OutOfMemoryError as e:
            torch.cuda.empty_cache()
            rec = {"status": "oom", "title": src, "error": str(e)[:200]}
        except Exception as e:
