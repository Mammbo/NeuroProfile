...
it speaks the same read API plus /analyze.
    ...
    python backend/serve.py --qdrant-path ./qdrant_data \
                            --timelines-dir ./data/timelines \
                            --videos-dir ./data/videos    # :8000

Run it from the repo root whatever the cwd: it chdirs there itself, because reducer.py
does np.load("ica/...") at import time against relative paths.
...
# Every entry point chdirs to the repo root before importing anything from backend/ —
# reducer.py does np.load("ica/...") at import time and storage's default paths are relative.
# This file lives IN backend/, so the root is two levels up, not one.
...
if __name__ == "__main__":
    ...
    if a.prewarm:
        prewarm_videos()
    if a.check:
        # a report, not a server — seed/prewarm first so --mock --check makes sense
